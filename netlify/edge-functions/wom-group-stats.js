...
export default async (request, _context) => {
  ...
  console.log(`Fetching stats for group ID: ${WOM_GROUP_ID}`);

  try {
    const response = await fetch(
      `https://api.wiseoldman.net/v2/groups/${WOM_GROUP_ID}/statistics`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Failed to fetch stats: ${errorText}` }),
        {
    ...
    return new Response(JSON.stringify(data), {
      headers: {
        ...
        "Netlify-Cache-Tag": `group-stats-${WOM_GROUP_ID}`,
      },
    });
  } catch (error) {
